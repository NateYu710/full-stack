...
const orderLabels = orders.map( ({product, price ,quantity}) =>  
...
let count = 0
for (const ComputerLanguage of searchHistory){
    count++
    console.log(`${count}. ${ComputerLanguage}`);
}
...
printTeam(allMembers[0], ...allMembers);
...
const {name : userName, role, nickname = '이름없음' } = user
